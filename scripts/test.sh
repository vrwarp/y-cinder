...
if [ -z "$TEST_ARGS" ]; then
  ...
  npx firebase emulators:exec --project demo-test-project "npx vitest run --no-file-parallelism --test-timeout=30000 --exclude debugger/tests/**/* --shard=1/3" || exit 1

  echo "--- RUNNING BATCH 2/3 ---"
  npx firebase emulators:exec --project demo-test-project "npx vitest run --no-file-parallelism --test-timeout=30000 --exclude debugger/tests/**/* --shard=2/3" || exit 1

  echo "--- RUNNING BATCH 3/3 ---"
  npx firebase emulators:exec --project demo-test-project "npx vitest run --no-file-parallelism --test-timeout=30000 --exclude debugger/tests/**/* --shard=3/3" || exit 1
else
  echo "Running specific tests: $TEST_ARGS"
  npx firebase emulators:exec --project demo-test-project "npx vitest run --no-file-parallelism --test-timeout=30000 --exclude debugger/tests/**/* $TEST_ARGS"
fi
